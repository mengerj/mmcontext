...
def run_eval_suite(cfg) -> None:
    ...
    for ds_cfg in cfg.datasets:
        print(f"\n=== Processing dataset: {ds_cfg.name} ===")
        ...
        for model_cfg in cfg.models:
            print(f"Processing model: {model_cfg.source} for dataset: {ds_cfg.name}")
            rows = []
            ...
            try:
                print(f"Loading embeddings from: {emb_dir / 'embeddings.parquet'}")
                emb_df = pd.read_parquet(emb_dir / "embeddings.parquet")
                print(f"✓ Loaded embeddings: {len(emb_df)} rows")

                E1 = np.vstack(emb_df["embedding"].to_numpy())
                print(f"✓ Stacked embeddings: {E1.shape}")

                print(f"Loading AnnData from: {emb_dir / 'subset.zarr'}")
                adata = ad.read_zarr(emb_dir / "subset.zarr")
                print(f"✓ Loaded AnnData: {adata.n_obs} obs × {adata.n_vars} vars")

            except FileNotFoundError as e:
                print(f"✗ Missing file for {ds_cfg.name}/{model_id}: {e}")
                print("  Skipping this model...")
                continue
            except Exception as e:
                print(f"✗ Error loading data for {ds_cfg.name}/{model_id}: {e}")
                print("  Skipping this model...")
                continue
            ...
            print(f"Running regular evaluators: {regular_evaluators}")
            ...
            for label_spec in label_specs:
                if label_spec.name not in adata.obs.columns:
                    print(f"  Skipping label {label_spec.name} - not found in adata.obs")
                    continue  # skip silently
                print(f"  Processing label: {label_spec.name} ({label_spec.kind})")
                ...
                if (label_spec.kind, label_spec.name) not in label_emb_cache:
                    ...
                    if path.exists():
                        try:
                            ...
                            print(f"    ✓ Loaded label embeddings for {label_spec.name}")
                        except Exception as e:
                            print(f"    ✗ Error loading label embeddings for {label_spec.name}: {e}")
                            label_emb_cache[(label_spec.kind, label_spec.name)] = None
                    else:
                        print(f"    - No label embeddings found for {label_spec.name}")
                        label_emb_cache[(label_spec.kind, label_spec.name)] = None
                E2 = label_emb_cache[(label_spec.kind, label_spec.name)]

                for ev_name in regular_evaluators:
                    print(f"    Running evaluator: {ev_name}")
                    try:
                        EvClass = get_evaluator(ev_name)
                        ev = EvClass()

                        if ev.requires_pair and E2 is None:
                            print(f"      Skipping {ev_name} - requires pair but E2 is None")
                            continue
                        ...
                        print(f"      ✓ {ev_name} completed, {len(result)} metrics")

                        if ev.produces_plot:
                            print(f"      Generating plots for {ev_name}")
                        ...
                        print(f"      ✗ Error in evaluator {ev_name}: {e}")
            ...
            if rows:
                ...
                print(f"✓ Saved {len(rows)} regular evaluation results")
            ...
            print(f"✓ Completed processing model {model_id} for dataset {ds_cfg.name}")

        print(f"✓ Completed processing dataset {ds_cfg.name}")
