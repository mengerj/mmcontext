"""Embed helpers for running inference with mmcontext (and mmcontext-like) models.

This module owns the *consumer-facing* glue for turning a loaded
:class:`~sentence_transformers.SentenceTransformer` (which may be a legacy
:class:`~mmcontext._legacy.mmcontextencoder.MMContextEncoder`) plus a dataset
into an embedding table. It encodes mmcontext's own model contract — downloading
numeric initial embeddings from a store link and registering them via
``register_initial_embeddings`` — so downstream consumers do not have to reach
into ``mmcontext._legacy`` themselves.

Consumers:

* ``mmcontext-benchmark`` — the ``MMContextAdapter`` legacy/v2 embed paths.
* ``sc_language_foundation_models`` — the per-model ``predict.py`` scripts
  (which still import from the back-compat :mod:`mmcontext.embed.model_utils`
  shim).

Only the helpers actually used by those consumers are kept here; the wider
multi-model orchestration removed in the v2.0 cleanup is intentionally *not*
restored.
"""
