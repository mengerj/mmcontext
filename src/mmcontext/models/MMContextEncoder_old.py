...
from mmcontext.models.MMContextProcessor_old import MMContextProcessor_old
...
class MMContextEncoder_old(nn.Module):
    """
    ...
    def __init__(
        ...
        self.processor = MMContextProcessor_old(**processor_params)
    ...
    def tokenize(self, texts, padding: str | bool = True) -> dict:
        ...
        for data in texts:
            if isinstance(data, int):
                ...
                # numpy arrays are sample indices for the datastore and can be used by OptimizedProcessor
                omics_reps.append(data)
                omics_text_info.append(0)
    ...
    def load(input_path: str, safe_serialization: bool = True):
        ...
        model = MMContextEncoder_old(**cfg)
        if safe_serialization and os.path.exists(os.path.join(input_path, "model.safetensors")):
            load_safetensors_model(model, os.path.join(input_path, "model.safetensors"))
        else:
    ...
    @property
    def tokenizer(self) -> MMContextProcessor_old:
